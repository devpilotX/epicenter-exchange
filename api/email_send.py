...
Env: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, FROM_EMAIL, NOTIFY_EMAIL."""
...
def _wrap(body_html: str) -> str:
    return f"<!doctype html><html><head><meta charset='utf-8'><style>{CSS}</style></head><body><div class='wrap'><div class='hdr'><h1>Epicenter Exchange</h1><p>Markets, math, and decisions — without the hype</p></div><div class='card'>{body_html}</div><div class='foot'>Contact: <a href='mailto:{NOTIFY_EMAIL}'>{NOTIFY_EMAIL}</a><br>© 2026 Epicenter Exchange · <a href='{BRAND_URL}'>epicenterexchange.com</a><br>Educational only. Not investment advice.</div></div></body></html>"
...
def send_contact_confirmation(name, email, topic, message, ticket_id):
    ...
    _send(email, f"We received your message — Ticket {ticket_id}", _wrap(body), text)
...
def send_newsletter_welcome(name, email, unsub_token, latest):
    ...
    unsub = f"https://api.epicenterexchange.com/newsletter/unsubscribe?token={unsub_token}"
    latest_block = ""
    if latest:
        latest_block = f"<div class='wait'><h3>Latest essay</h3><p style='margin:0 0 6px'><a href='{latest['url']}' style='color:#0B1F3A;font-weight:700;font-size:18px;text-decoration:none'>{latest['title']}</a></p><p style='margin:0;color:#475569'>{latest.get('summary','')}</p><p style='margin:12px 0 0'><a class='btn' href='{latest['url']}'>Read essay →</a></p></div>"
    body = f"<p class='greet'>{greet}</p><p>You're subscribed to <strong>Epicenter Exchange</strong> — one math-grounded essay every fortnight on India, US, UK, and crypto markets. No tracking, no spam, one-click unsubscribe.</p>{latest_block}<div class='wait'><h3>What you'll get</h3><ul><li>One essay every other Sunday — long-form, data-grounded</li><li>Occasional new-tool notes (max once a month)</li><li>Never your data sold or shared</li></ul></div><p style='font-size:13px;color:#64748B;margin-top:24px'>Not interested? <a href='{unsub}' style='color:#C9A227'>Unsubscribe in one click</a>.</p>"
    text = f"{greet}\n\nYou're subscribed to Epicenter Exchange. One essay every fortnight.\n\n{(latest['title']+': '+latest['url']) if latest else ''}\n\nUnsubscribe: {unsub}"
    _send(email, "Welcome to Epicenter Exchange", _wrap(body), text)
